...
//   // Helper for validation of input fields
//   const validateInputs = () => {
//     const requiredFields = [sellerDetails, billingDetails, shippingDetails, orderDetails, invoiceDetails];
//     for (let fieldSet of requiredFields) {
...
//     const computedItems = items.map(item => {
//       // Handle situation where discount is not provided
//       const discount = item.discount ? item.discount : 0;
//       const netAmount = ((item.unitPrice * item.quantity) - (item.unitPrice * item.quantity) * (discount * 0.01));
//       const taxType = placeOfSupply === placeOfDelivery ? 'CGST & SGST' : 'IGST';
//       const taxAmount = taxType === 'CGST & SGST'
//         ? { CGST: netAmount * 0.09, SGST: netAmount * 0.09 }
//         : { IGST: netAmount * 0.18 };
...
//       <form onSubmit={handleSubmit}>
        ...
        // {/* Repeat similar for Billing Details, Shipping Details, etc. */}
        // <h2>Billing Details</h2>
        // <div className="form-group">
        //   <input type="text" placeholder="Name" value={billingDetails.name} onChange={(e) => handleInputChange(e, 'billingDetails', 'name')} required />
        //   <input type="text" placeholder="Address" value={billingDetails.address} onChange={(e) => handleInputChange(e, 'billingDetails', 'address')} required />
...
const Input = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Now round the total amount to 2 decimal places
  ...
  return (
    showInvoice == null ? (
      <form onSubmit={handleSubmit}>
        ...
        {/* Other form fields for seller details, billing details, etc. */}
